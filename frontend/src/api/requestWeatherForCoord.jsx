...
export default async function requestWeatherForCoord(coordinates) {
  try {
    ...
    const response = await abortController(
      `https://api.open-meteo.com/v1/forecast?latitude=${coordinates[1]}&longitude=${coordinates[0]}&hourly=temperature_2m,apparent_temperature,relative_humidity_2m,wind_speed_10m,wind_direction_10m,weather_code&timezone=auto`,
      10000
    );
